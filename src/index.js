...
export function createContext (ctx) {
  const { token } = ctx
  const newCtx = Object.assign({}, ctx, { bearer: `Bearer ${token}` })
  if (!newCtx.fetch) newCtx.fetch = window.fetch
  return newCtx
}
...
function fetchWebsite ({ fetch, clusterURL, websiteID, bearer }) {
  return fetch(
...
function fetchPageState ({ fetch, clusterURL, websiteID, bearer }, id) {
  const promises = [
    fetch(
...
function fetchObjectState (ctx, type, id) {
  const { fetch, isNew, websiteID, clusterURL, bearer } = ctx
  const promises = [
    isNew
...
function triggerBuild ({ fetch, clusterURL, websiteID, bearer }) {
